...
class _RestaurantManageScreenState extends State<RestaurantManageScreen> {
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 20),
                    _buildLocation(widget.restaurant),
                    const SizedBox(height: 20),
                    _buildManageButtons(),
                    const SizedBox(height: 10),
                    IconButtonCustom(
        ...
        IconButtonCustom(
          ...
          onPressed: () async {
            final updatedRestaurant = await Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => RestaurantEditScreen(restaurant: widget.restaurant),
              ),
            );
            ...
            if (updatedRestaurant != null && updatedRestaurant is Map<String, dynamic>) {
              setState(() {
                ...
                widget.restaurant.addAll(updatedRestaurant);
              });
            }
          },
        ),
        ...
        IconButtonCustom(
          ...
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => EditTablesScreen(
                  restaurant: widget.restaurant,
                ),
              ),
            );
  ...
  Widget _buildServices(Map<String, dynamic>? restaurantData) {
    final List availableServices = restaurants.firstWhere(
      (resto) => resto['id'] == restaurantData?['id'], // 🔥 Busca el restaurante por `id`
    ...
    return Column(
      children: [
        ...
        if (availableServices.isEmpty) const Text("No hay servicios disponibles para este restaurante."),
      ],
    );
  }
  ...
  Widget _buildInfoRows(BuildContext context, Map<String, dynamic>? restaurantData) {
    final Map<String, dynamic> restaurantInfo = restaurants.firstWhere(
      (restaurant) => restaurant['id'] == restaurantData?['id'], // 🔥 Busca el restaurante por `id`
      orElse: () => {
        'phone': 'No disponible',
